...
class MainActivity : AppCompatActivity(), TextToSpeech.OnInitListener {
    ...
    private fun speak() {
        ...
        if (message.isEmpty()) {
            findViewById<TextView>(R.id.tvStatus).text = getString(R.string.tts_placeholder)
            message = getString(R.string.tts_messageWarning)
        }
    ...
    override fun onInit(status: Int) {
        if (status == TextToSpeech.SUCCESS) {
            ...
            findViewById<TextView>(R.id.tvStatus).text = getString(R.string.tts_notEnabled)
        }
    }
